...
const checkInstructorTelNo = (value) => {
  	return axios.post('/staff/checkInstructorTelNo.proc', {telNo: value , originTelNo : $('#originCellPhone').val()}).then((response) => {
      ...
      this.$validator.validate().then((result) => {
        if (result) {
			...
			if (acceessableCount < 0 ) {
				...
				axios.post('/staff/nsUpdate.proc', formdata,{
					  headers: {
...
function cancel(){
	if(confirm('원어민 정보 수정을 취소하고 리스트로 돌아가시겠습니까?')){
		location.href = '/staff/nsList.view';	
	}
}